...
- `const int8_t sound_data[4] = {-30, -15, -15, -15};` - данные настройки звука по входам по умолчанию; 
  - значения в пределах -60..15; нумерация входов обратная - `4, 3, 2, 1`; 
  ...
  - данные используются, если значение в `EEPROM_INDEX_FOR_VALIDATE_FLAG` != `VALIDATE_FLAG;`
...
Для выхода из сервисного режима нужно перезапустить музыкальный центр.
