...
Модуль, позволяющий использовать микросхему темброблока **tda7439** вместо **tda7339** (которая, как оказалось, практически не существует в природе, найти ее можно разве что в старой технике под утилизацию; у меня найти не получилось). Модуль построен на базе МК **ATmega328pb** и запрограммирован в среде Arduino IDE.
...
Для работы с **Atmega328pb** в среде Arduino IDE используется аддон [**MiniCore**](https://github.com/MCUdude/MiniCore/tree/master). 
...
При наличии внешнего mp3-модуля и, соответственно, управляющей им кнопки появляется возможность настройки громкости воспроизведения для каждого канала "на слух". Это позволяет избавиться от подбора нужной максимальной громкости звука на входах путем перепрошивки МК с разными параметрами настройки звука (см `sound_data[4]`).
